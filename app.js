...
addEventListener('submit', function(event) { event.preventDefault();
   ...
   document.getElementById('name').value; alert('Signup successful!'); });

function openSignup() { document.getElementById('signup')
  .scrollIntoView({ behavior: 'smooth' }); }

function sendMoney() { let history = 
document.getElementById('history-list');

 let newTransaction = document.createElement('li');
  newTransaction.innerText = 'Sent $100 to another user'; 
  history.appendChild(newTransaction); alert('Money sent successfully!'); }

function donateZakat() { let history =
   document.getElementById('history-list');
    let newTransaction = document.createElement('li'); 
...
{ let amount = prompt('Enter withdrawal amount:'); 
  if (amount && !isNaN(amount)) { let history =
     document.getElementById('history-list'); 
     let newTransaction = document.createElement('li'); 
...
function scanQR() { alert('QR Payment feature coming soon!'); }
